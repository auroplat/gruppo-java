...
public final class PezziLiberiApiHttpHandler extends BaseHttpHandler {
    ...
    public PezziLiberiApiHttpHandler(
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            String valoreTipo = parametriQuery(exchange)
                    .get("tipo");

            if (valoreTipo == null || valoreTipo.isBlank()) {
            ...
            TipoPezzo tipo = TipoPezzo.valueOf(
                    valoreTipo.trim().toUpperCase(Locale.ROOT)
            );

            List<Pezzo> pezzi = controller.trovaPezziLiberi(tipo);
            sendJson(exchange, 200, creaJson(pezzi));

        } catch (IllegalArgumentException e) {
            sendJson(
                    exchange,
                    400,
                    "{\"errore\":\"" + escape(e.getMessage()) + "\"}"
            );
            ...
            sendJson(
                    exchange,
                    500,
                    "{\"errore\":\"Impossibile leggere i pezzi liberi\"}"
            );
        }
    }
    ...
    private String pezzoJson(Pezzo pezzo) {
        return "{" +
                "\"idPezzo\":\"" + escape(pezzo.getIdPezzo()) + "\"," +
                "\"tipoPezzo\":\"" + escape(pezzo.getTipo().name()) + "\"," +
                "\"kmMax\":" + pezzo.getKmMax() + "," +
                "\"tempoMax\":" + pezzo.getTempoMax() +
                "}";
    }

    private Map<String, String> parametriQuery(HttpExchange exchange) {
